...
public class SysPluginServiceImpl extends BaseServiceImpl<SysPlugin> implements SysPluginService {
    ...
    @Autowired
    private HexoBootPluginManager hexoBootPluginManager;
    ...
    public String installPlugin(String originalFilename, InputStream inputStream) throws GlobalException {
        ...
        Path pluginsRoot = this.hexoBootPluginManager.getPluginsRoot();
        ...
        try {
            ...
            pluginId = this.hexoBootPluginManager.loadPlugin(Paths.get(filePath));
            PluginWrapper pluginWrapper = this.hexoBootPluginManager.getPlugin(pluginId);
            PluginDescriptor descriptor = pluginWrapper.getDescriptor();

            // 检测版本要求
            this.checkVersion(descriptor);

            PluginState pluginState = this.hexoBootPluginManager.startPlugin(pluginId, filePath);
            ...
            this.sysPluginMapper.insert(plugin);

        } catch (Exception e) {
            this.hexoBootPluginManager.deletePlugin(pluginId);
    ...
    public void updatePlugin(SysPlugin plugin) throws GlobalException {
        ...
        if (plugin.getState()) {
            this.hexoBootPluginManager.startPlugin(pluginId, dbPlugin.getFilePath());
        } else {
            this.hexoBootPluginManager.stopPlugin(pluginId);
        }
    ...
    public void uninstallPlugin(Integer id) throws GlobalException {
        ...
        if (pluginFile.exists()) {
            ...
            try {
                if (this.hexoBootPluginManager.checkPlugin(pluginId)) {
                    this.hexoBootPluginManager.deletePlugin(pluginId);
                } else {
                    boolean deleteQuietly = false;
                    int tryCount = 0;
    ...
    public boolean checkPlugin(String pluginId) throws GlobalException {
        ...
        SysPlugin dbPlugin = this.sysPluginMapper.selectOneByExample(example);
    ...
    private SysPlugin getPluginByName(String name) {
        ...
        return this.sysPluginMapper.selectOneByExample(example);
    }
}
